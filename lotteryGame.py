...
def main ():
    ...
    won = False              # flag; reset to true if the player wins
    ...
    while (balance != 0 and not won):
        ...
        while numPicked < MIN or numPicked > MAX:   #use for the input check...a good function to use
            numPicked = int(input("Invalid number! Please try again -> "))
        
        # get number drawn by computer and print it
        numDrawn = random.randint(MIN, MAX)
    ...
    print ("\nGame over !!!")
    if won:
        print ("You spent $",  START_BALANCE - balance, end="")
